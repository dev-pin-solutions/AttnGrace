vogue/******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(12), __webpack_require__(32).setImmediate))
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(12), __webpack_require__(34)))
...
function () {
  function ProductGallery(el, settings) {
    ...
    this.viewport = this.el.querySelector('[data-product-gallery]');
  ...
  ProductGallery_createClass(ProductGallery, [{
    ...
    value: function initFlickity() {
      ...
      this.flickity = new js_default.a(this.viewport, {
        pageDots: false,
        prevNextButtons: this.figures.length > 1,
        arrowShape: svgPath('icon-flickity-arrow'),
        wrapAround: true
      });
      this.selected = this.figures[0];

      var playIfModel = function playIfModel(figure) {
        if (figure.dataset.mediaType === 'model') {
          var model = _this2.media.filter(function (media) {
            return media.figure === figure;
          })[0];
      ...
      var autoplayVideo = function autoplayVideo(figure) {
        if (figure.dataset.mediaType === 'video' || figure.dataset.mediaType === 'external_video') {
          var video = _this2.media.filter(function (media) {
            return media.figure === figure;
          })[0];

          if (video) {
            if (_this2.settings.enableVideoAutoplay) {
              video.play();
            }
      ...
      this.flickity.on('change', function (index) {
        var figure = _this2.figures[index];
        _this2.selected = figure;

        _this2._pauseMedia();

        autoplayVideo(figure);
        playIfModel(figure);
      ...
      this.flickity.on('staticClick', function (event, pointer, cellElement) {
        return playIfModel(cellElement);
      });
    }
  }, {
    ...
    value: function _onModelLibraryLoad() {
      ...
      modelFigures.forEach(function (figure) {
        var modelEl = figure.querySelector('model-viewer');
        var player = new Shopify.ModelViewerUI(modelEl, {
          controls: controls
        });
        ...
        _this3.events.register(modelEl, 'shopify_model_viewer_ui_toggle_play', onPlayListener);

        _this3.events.register(modelEl, 'shopify_model_viewer_ui_toggle_pause', onPauseListener);
      });
    }
  }, {
    key: "_onViewInYourSpaceClick",
    value: function _onViewInYourSpaceClick(target) {
      if (!this.selected || target.dataset.shopifyModel3dId === this.selected.dataset.media) return;
    ...
    value: function _onVideoLibraryLoad() {
      ...
      videoFigures.forEach(function (videoFigure) {
        var player = new Shopify.Plyr(videoFigure.querySelector('video'), {
          loop: {
            active: _this4.settings.enableVideoLooping
          }
        });
        ...
        _this4.events.register(videoFigure, 'play', function () {
    ...
    value: function _onExternalVideoLibraryLoad() {
      ...
      videoFigures.forEach(function (videoFigure) {
        ...
        var video = {
          ...
          pause: function pause() {
            return playerReady && player.pauseVideo();
          },
          play: function play() {
        ...
        _this5.events.register(player, 'onStateChange', function (_ref) {
          ...
          if (data === YT.PlayerState.ENDED && _this5.settings.enableVideoLooping) {
            video.restart();
...
function () {
  function Product(theme) {
    ...
    this.events = new EventHandler_default.a();
    this.galleryEnableVideoLooping = this.$productContainer.length && this.$productContainer[0].dataset.galleryEnableVideoLooping === 'true'; // Sidebar settings
  ...
  Product_createClass(Product, [{
    ...
    key: "_pauseGalleries",
    value: function _pauseGalleries() {
      var exclude = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      this.products.filter(function (product) {
        return product.productGallery !== exclude;
      }).forEach(function (product) {
        return product.productGallery.pause();
      });
    }
  }, {
    key: "_generateProductGallery",
    value: function _generateProductGallery(el) {
      var _this3 = this;

      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
      return new ProductGallery_ProductGallery(el, _objectSpread({
        enableVideoLooping: this.galleryEnableVideoLooping,
        pauseOtherGalleries: function pauseOtherGalleries(currentGallery) {
          return _this3._pauseGalleries(currentGallery);
        }
      }, options));
    }
    /**
    ...
    value: function _addProducts(products) {
      ...
      jquery_default()(products).each(function (index, el) {
        ...
        if (addToPage) {
          _this4.$productContainer.find('.product:last').after($el);
        ...
        var $gallery = $el.find('.product-gallery');
        var originalGallery = $gallery[0].cloneNode(true);

        var productGallery = _this4._generateProductGallery($el[0]); // Cache elements so we can access them quickly in the scroll handler
        ...
        _this4.products.push({
          ...
          originalGallery: originalGallery
        });
    ...
    value: function _bindEvents() {
      ...
      this.$window.on('scroll.vogue-product', function () {
        if (_this5._isMobile() || _this5._isSingleSidebarTablet(false) || _this5._isSingleSidebarTablet(true)) return;

        var percent = _this5.$window.scrollTop() / (_this5.$document.height() - _this5.$window.height());

        if (percent < 0.6) return;

        _this5._fetchProducts(4);
      });
      this.$window.on('shopify:section:unload', function () {
        if (_this5.$extendedProduct && _this5.$extendedProduct.length) {
          _this5._closeExtendedDetails(_this5.$extendedProduct);
        }
      ...
      this.$body.on('click.vogue-product', this.extendedMobileSelector, function (event) {
        ...
        var $el = $target.parents(_this5.productSelector).find('.product-details');

        _this5._ensureElementInViewport($el[0]);
      }); // Open the extended details panel

      this.$body.on('click.vogue-product', this.extendedOpenSelector, function (event) {
        event.preventDefault();
        var $target = jquery_default()(event.currentTarget);
        _this5.$extendedProduct = $target.parents(_this5.productSelector);

        _this5._openExtendedDetails(_this5.$extendedProduct);

        var newEvent = new CustomEvent('vogue:loadRecommended', {
          detail: {
            productElement: _this5.$extendedProduct[0]
          }
        });
      ...
      this.$window.on('resize.vogue-product', function () {
        if (_this5.$extendedProduct && (_this5._isMobile() || _this5._isSingleSidebarTablet(false) || _this5._isSingleSidebar(true))) {
          _this5._closeExtendedDetails(_this5.$extendedProduct);
    ...
    value: function _openExtendedDetails($product) {
      ...
      this.theme.sidebar.toggleExtended(function () {
        ...
        $product.find('.recommended-products').revealer('show'); // Create slideshow

        _this6._createProductSlideshow($product); // Set up tab focus loop between details and slideshow,
        // which are in totally separate parts of the DOM.


        jquery_default()(_this6.slideshow.el).one('revealer-show', function () {
          var details = $product[0].querySelector('[data-product-details-panel]');
          var focusableDetails = focusable(details);
          var slideshow = _this6.slideshow.el;
          var focusableSlideshow = focusable(slideshow);
          trapFocus($product[0].querySelector('[data-product-details-panel]')); // We use keydown events rather than blur to avoid issues
          // with video/model media, which changes focus and would
          // otherwise trigger these event listeners.

          _this6.focusCycleDetails = _this6.events.register(focusableDetails[focusableDetails.length - 1], 'keydown', function (event) {
            if (event.key !== 'Tab') return;
            trapFocus(slideshow);
          });
          _this6.focusCycleSlideshow = _this6.events.register(focusableSlideshow[focusableSlideshow.length - 1], 'keydown', function () {
            if (event.key !== 'Tab') return;
            trapFocus(details);
          });
        });
      ...
      this.$window.on('resize.vogue-extended-resize', function () {
        _this6._ensureElementInViewport(gallery, false);
      });
    }
    /**
    ...
    value: function _closeExtendedDetails($product) {
      ...
      this.events.unregister(this.focusCycleDetails);
      this.events.unregister(this.focusCycleSlideshow);
      removeTrapFocus();

      this._destroyProductSlideshow();

      $product.find('.product-options').slideUp();
      $product.find('.recommended-products').revealer('hide'); // Animate out the extended details
      ...
      $product.find('.product-description-full').revealer('hide').one('revealer-hide', function () {
        ...
        _this7.theme.sidebar.toggleExtended(null, false);
    ...
    value: function _createProductSlideshow($product) {
      var clonedGallery = this.products.filter(function (product) {
        return product.$el[0] === $product[0];
      })[0].originalGallery.cloneNode(true);
      var $slideshow = jquery_default()('<div>').addClass('product-slideshow').revealer('hide', true);
      $slideshow[0].appendChild(clonedGallery);
      this.slideshow = {
        el: $slideshow[0],
        productGallery: this._generateProductGallery($slideshow[0], {
          enableVideoAutoplay: true
        })
      };
      this.$body.append($slideshow);
      dist_index_es.watch($slideshow[0]);
      this.slideshow.productGallery.initFlickity(); // Select first available variant media

      var variant = this._getSelectedProductVariant($product);

      this.slideshow.productGallery.selectMediaByVariant(variant); // Show the slideshow
    ...
    value: function _destroyProductSlideshow() {
      ...
      jquery_default()(slideshow).revealer('hide').one('revealer-hide', function () {
        _this8.slideshow.productGallery.unload();

        slideshow.parentNode.removeChild(slideshow);
        _this8.slideshow = null;
      });
    }
    /**
    ...
    value: function _stickyTick() {
      ...
      if (!this.stickyTicking) {
        window.requestAnimationFrame(function () {
          _this9._offsetSticky(_this9.lastKnownScrollPosition, _this9.currentKnownScrollPosition);

          _this9.stickyTicking = false;
        });
    ...
    value: function _positionTick() {
      ...
      if (!this.positionTicking) {
        window.requestAnimationFrame(function () {
          _this10._updateProductPositions();

          _this10.positionTicking = false;
        });
    ...
    value: function _updateProductPositions() {
      ...
      var $sidebar = jquery_default()('.product-main-sidebar');
      ...
      var headerHeight = sidebarEnabled ? $sidebar.outerHeight() : 0;
      ...
      this.products.forEach(function (product) {
        ...
        if (productRect.top < screenHeightHalf && productRect.bottom > screenHeightHalf) {
          _this11._setHistory(product);
        }

        if (_this11._isMobile() || _this11._isSingleSidebarTablet(true)) {
          ...
          product.$mobileHeader.css({
            ...
            right: _this11.gridSpacing,
            bottom: 0,
            left: _this11.gridSpacing,
            display: isInFrame ? '' : 'none'
          });
        } else if (!pinDetailsToTop && productRect.bottom - detailsHeight - _this11.gridSpacing * 2 - headerHeight < 0) {
          // Pin to bottom of the product
          product.$details.css({
            position: 'absolute',
            top: 'auto',
            left: -sidebarOffset,
            bottom: _this11.gridSpacing,
            maxHeight: height
          });
          ...
          product.$details.css({
            position: 'fixed',
            top: headerHeight + _this11.gridSpacing,
            left: _this11.gridSpacing,
            bottom: 'auto',
            maxHeight: height
          });
          ...
          product.$details.css({
            position: 'absolute',
            top: _this11.gridSpacing + topOffset,
            left: leftOffset,
            bottom: 'auto',
            maxHeight: height
          });
    ...
    value: function _setHistory(product) {
      ...
      var productUrl = product.$details[0].dataset.productUrl;
    ...
    value: function _initProductOptions() {
      ...
      this.$body.on('change.vogue-product', '[data-option-input]', function (event) {
        ...
        var variant = _this12._getSelectedProductVariant($product);

        _this12._updateProductVariant($product, variant);
      });
    }
  }, {
    ...
    value: function _selectVariantImage($product, variant) {
      if (!variant || !variant.featured_media) return; // Grab selected media element

      var $productImage = $product.find("[data-media=\"".concat(variant.featured_media.id, "\"]"));
      if (!$productImage.length) return; // Change slideshow slide if we are in mobile

      if (this.slideshow) {
        this.slideshow.productGallery.selectMediaByVariant(variant);
        return;
      }
      ...
      this._pauseGalleries(); // Fade out non-selected images


      var $productImages = $product.find('[data-media]');
      $productImages.not($productImage).addClass('product-media-fade'); // Scroll to selected image
      ...
      jquery_default()('html,body').animate({
        ...
        setTimeout(function () {
          $productImages.removeClass('product-media-fade');
        }, 333);
      });
    }
  }, {
...
var just_debounce = __webpack_require__(3);
...
var vanilla_modal_dist = __webpack_require__(11);
...
var dist_player = __webpack_require__(15);
...
var twitterFetcher_min = __webpack_require__(16);
...
var ProductRecommendations_ProductRecommendations =
/*#__PURE__*/
function () {
  function ProductRecommendations(section) {
    ...
    this.recommendationContainer = document.querySelector('[data-product-recommendations-container]');
    ...
    this._loadMoreRecommendations = this._loadMoreRecommendations.bind(this);

    this._loadRecommendations();

    window.addEventListener('shopify:section:load', this._reloadRecommendations);
    window.addEventListener('vogue:loadRecommended', this._loadMoreRecommendations);
  }
  ...
  ProductRecommendations_createClass(ProductRecommendations, [{
    ...
    value: function _loadRecommendations() {
      ...
      var recommendUrl = "/recommendations/products?section_id=product-recommendations&limit=".concat(this.limit, "&product_id=").concat(this.productId);
      shopify_asyncview_dist_index_es.load(recommendUrl).done(function (_ref) {
        var html = _ref.html;

        if (!html.length) {
          _this.recommendationContainer.innerHTML = '';
          return;
        }
        ...
        var productItems = _this.recommendationContainer.querySelectorAll('.recommended-product');

        if (productItems.length) {
          var moneyItems = _this.recommendationContainer.querySelectorAll('.price');
    ...
    } // This method is only called inside of the theme editor when the user changes their setting
    // for enable single product view
    ...
    value: function _reloadRecommendations() {
      ...
      this._loadRecommendations();
    } // This method is to be called when we need to load more recommended products for products other
    // than the first product in the collection.

  }, {
    key: "_loadMoreRecommendations",
    value: function _loadMoreRecommendations(event) {
      var productElement = event.detail.productElement;
      var productId = productElement.getAttribute('data-product-id');
      this.recommendationContainer = productElement.querySelector('[data-product-recommendations-container]');
    ...
    value: function unload() {
      ...
      window.removeEventListener('vogue:loadRecommended', this._loadMoreRecommendations);
    }
  }]);
...
function () {
  ...
  Cart_createClass(Cart, [{
    ...
    value: function init() {
      ...
      if (this.cartForm && this.cartForm.length) {
    ...
    value: function _updateCart(thisTimeoutId) {
      ...
      shopify_asyncview_dist_index_es.load('/cart', '_ajax').done(function (_ref) {
        ...
        _this3.cartTable.querySelectorAll('[data-money]').forEach(function (money) {
          return dist_default.a.update(money);
        });
        ...
        _this3.cartDiscounts.querySelectorAll('[data-money]').forEach(function (money) {
          return dist_default.a.update(money);
        }); // Update total
  ...
  Sidebar_createClass(Sidebar, [{
    ...
    value: function _bindEvents() {
      ...
      this.$body.on('click', '.navigation-parent-link', function (event) {
        ...
        $link.toggleClass("active-submenu");
...
function () {
  ...
  Vogue_createClass(Vogue, [{
    ...
    value: function _init() {
      ...
      this.sections.register('product-recommendations', function (section) {
        return new ProductRecommendations_ProductRecommendations(section);
      });
      FlickityTouchFix();
...
/***/ })
/******/ ]);
//# sourceMappingURL=vogue.js.map?1582567606998
